...
export default function DashboardPage() {
  ...
  const [agendamentosSemanaPrevisao, setAgendamentosSemanaPrevisao] = useState<any[]>([]);
...
setMostrarFinanceiro(podeVerFinanceiro);
  ...
  const intervaloSemanaPrevisao = intervaloSemanaPorData(dataFim || dataInicio);
...
setPromocoesAtivas(promocoesValidas);
  ...
  function metodosPagamentoTexto(agendamento: any) {
    ...
    if (Array.isArray(pagamentosPossiveis)) {
      return pagamentosPossiveis
        .map((pagamento: any) => pagamento?.metodoPagamento || pagamento?.formaPagamento || '')
        .filter(Boolean)
        .join(' | ');
    }
  ...
  function resumoAjusteFechamentoSalvo(agendamento: any, resumoAdicionaisPreferencial?: any) {
    const adicionais = resumoAdicionaisDoAgendamento(
      agendamento,
      resumoAdicionaisPreferencial
    );

    const valorOriginal = valorServicoPrincipalOriginal(agendamento) + adicionais.total;
    ...
    if (valorOriginal > 0 && valorFinal > 0 && desconto <= 0 && acrescimo <= 0) {
...
function promocaoDoAgendamento(agendamento: any) {
  ...
  return promocoesAtivas.find((promo: any) =>
    Array.isArray(promo.servicos) &&
    promo.servicos.some((item: any) => item.servicoId === servicoId || item.servico?.id === servicoId)
  );
}
  ...
  function valorDescontoFechamento() {
    const valorInformado = valorNumerico(formFinalizacao.valorDescontoAtendimento);

    if (formFinalizacao.tipoDescontoAtendimento === 'percentual') {
      const financeiroBase = agendamentoSelecionado
        ? resumoFinanceiroAtendimento(agendamentoSelecionado, resumoServicosAdicionais)
        : { pendente: 0 };
  ...
  function valorAcrescimoFechamento() {
    const valorInformado = valorNumerico(formFinalizacao.valorAcrescimoAtendimento);
...
if (
  ...
  const pendenteAjustadoFechamentoAtual = pendenteAjustadoFechamento(agendamentoSelecionado);
  ...
  if (pendenciaComCredito.pendenteFinal > 0) {
    const valorDebitoCliente = valorNumerico(formFinalizacao.valorDebitoCliente);
  ...
  try {
    ...
    const res = await fetch('/api/agendamentos/finalizar', {
      ...
      body: JSON.stringify({
        ...
        observacaoAjusteAtendimento: formFinalizacao.observacaoAjusteAtendimento,
...
async function confirmarPresenca(agendamento: any) {
  ...
  async function carregarAtendimentosCentral(empresaId = empresa?.id) {
    ...
    try {
      ...
      const res = await fetch(`/api/dashboard/agendamentos?${params.toString()}`, {
        cache: 'no-store',
      });
  ...
  const agendamentosCentralUnicos = removerAgendamentosDuplicados(agendamentosCentral);
  const agendamentosSemanaPrevisaoUnicos = removerAgendamentosDuplicados(agendamentosSemanaPrevisao);
  ...
  const agendamentosSemana = agendamentosOperacionaisBase.filter((agendamento) =>
    estaNaSemanaAtual(agendamento.dataHoraInicio)
  );

  const agendamentosSemanaOperacionalBase = agendamentosOperacionaisBase;

  const agendamentosSemanaOperacional = agendamentosSemanaOperacionalBase.filter(
    (agendamento) =>
      estaNaSemanaAtual(agendamento.dataHoraInicio) &&
      agendamento.status !== 'cancelado'
  );
...
const dataFim = formatarDataInputLocal(fim);
  ...
  const comissoesPeriodo = valorNumerico(resumo.comissaoTotal || resumo.comissoesTotal);
  const lucroLiquidoPeriodo =
    valorNumerico(resumo.lucroLiquido) ||
    Math.max(faturamentoPeriodo - custoOperacionalPeriodo - comissoesPeriodo, 0);

  const atendimentosPagosPeriodo = agendamentosPeriodoValidos.filter((agendamento) => {
  const financeiro = resumoFinanceiroAtendimento(agendamento);
...
const ticketMedioPeriodo =
  ...
  const taxaCancelamentoPeriodo =
    agendamentosUnicos.length > 0
      ? Math.round((agendamentosCanceladosPeriodo.length / agendamentosUnicos.length) * 100)
      : 0;
...
const comissoesPeriodoEstimadas = agendamentosSemanaPrevisaoValidos.reduce(
  ...
  const clientesUnicosPeriodo = new Set(
    agendamentosPeriodoValidos.map((agendamento) =>
      agendamento.clienteId ||
      agendamento.cliente?.id ||
      agendamento.Cliente?.id ||
    ...
    >
      <style jsx global>{`
        ...
        @media (max-width: 760px) {
          ...
          @media (max-width: 760px) {
            .dashboard-modal-box-mobile:has(.dashboard-atendimento-selecionado-mobile) .dashboard-pesquisa-central-mobile,
            .dashboard-modal-box-mobile:has(.dashboard-atendimento-selecionado-mobile) .dashboard-filtros-central-mobile,
            .dashboard-modal-box-mobile:has(.dashboard-atendimento-selecionado-mobile) .dashboard-lista-central-mobile {
            ...
            .dashboard-fechamento-box-mobile {
              ...
              box-shadow: 0 18px 44px rgba(34,197,94,0.10) !important;
            }
            ...
            .dashboard-painel-atendimento-mobile > div:not(.dashboard-fechamento-box-mobile):not(.dashboard-servicos-box-mobile) {
              position: relative !important;
              z-index: 2 !important;
            }
          }
      ...
      <section className="dashboard-filter-mobile" style={filtroEnterpriseBox}>
        ...
        <div className="dashboard-filter-actions-compact" style={filtroCompactActions}>
          <button onClick={filtrarHojeDashboard} style={botaoFiltroEnterprise}>Hoje</button>
          <button onClick={() => filtrarUltimosDiasDashboard(7)} style={botaoFiltroEnterprise}>7 dias</button>
          <button onClick={() => filtrarUltimosDiasDashboard(30)} style={botaoFiltroEnterprise}>30 dias</button>
          <button
            type="button"
            onClick={() => setMostrarFiltroAvancado((valor) => !valor)}
            style={mostrarFiltroAvancado ? botaoFiltroEnterprisePrimario : botaoFiltroEnterprise}
          >
            Filtros
          </button>
        </div>
        ...
        {mostrarFiltroAvancado && (
          <div className="dashboard-filter-grid-mobile" style={filtroEnterpriseGrid}>
            <div>
            ...
            <button onClick={aplicarFiltroDashboard} style={botaoFiltroEnterprisePrimario}>
              Aplicar filtro
            </button>
          </div>
        )}
      ...
      <section className="dashboard-mobile-compact-stack" style={mobileCompactStack}>
        {usuarioPodeVerFinanceiro() ? (
          <div style={mobileCompactPanel}>
            <div style={mobileCompactHeaderLine}>
              ...
              <a href="/financeiro" style={mobileCompactLink}>Ver</a>
            </div>

            <div style={mobileFinanceMiniGrid}>
              <MobileMetric label="Recebido" value={valorFinanceiro(faturamentoPeriodo)} tone="#22c55e" />
              <MobileMetric label="Pendente" value={valorFinanceiro(financeiroPeriodo.pendente)} tone="#f59e0b" />
              <MobileMetric label="Previsão" value={valorFinanceiro(previsaoPeriodo)} tone="#8b5cf6" />
              <MobileMetric label="Pré-pago" value={valorFinanceiro(prePagamentosPeriodo)} tone="#06b6d4" />
            </div>
          </div>
        ) : (
          <div style={mobileCompactPanel}>
            ...
            <div style={mobileMutedBox}>Valores financeiros ocultos para este usuário.</div>
          </div>
        )}
        ...
        <div style={mobileCompactPanel}>
          <div style={mobileCompactHeaderLine}>
            ...
            <a href="/agenda" style={mobileCompactLink}>Agenda</a>
          </div>
          ...
          {agendamentosHoje.length === 0 ? (
            ...
            <div style={mobileAgendaStack}>
              {agendamentosHoje.slice(0, 4).map((agendamento) => {
                ...
                return (
                  <div key={agendamento.id} style={mobileAgendaItem}>
                    <div style={mobileAgendaTime}>{formatarHorario(agendamento.dataHoraInicio)}</div>
                    ...
                    <div style={mobileAgendaInfo}>
                      ...
                      <small>
                        {mostrarFinanceiro ? dinheiro(financeiro.total) : '••••••'} ·{' '}
                    ...
                    <div style={mobileAgendaActions}>
                      ...
                      {agendamento.status !== 'cancelado' && agendamento.status !== 'concluido' && (
                        <button
                          type="button"
                          onClick={() => abrirFechamentoAtendimento(agendamento)}
              ...
              {agendamentosHoje.length > 4 && (
                <button type="button" onClick={abrirModalReagendamento} style={mobileFullButton}>
        ...
        <div style={mobileCompactPanelSoft}>
          <div style={mobileCompactHeaderLine}>
            ...
            <button type="button" onClick={abrirModalReagendamento} style={mobileTextButton}>Central</button>
          </div>

          <div style={mobileAlertStack}>
            <MobileInsight icon="💰" label="Pendências" value={valorFinanceiro(financeiroPeriodo.pendente)} />
            <MobileInsight icon="🏆" label="Top serviço" value={servicoCampeao ? `${servicoCampeao.nome} · ${servicoCampeao.quantidade}x` : 'Sem dados'} />
            <MobileInsight icon="👤" label="Profissional" value={profissionalDestaque ? `${profissionalDestaque.nome} · ${profissionalDestaque.quantidade}x` : 'Sem dados'} />
            <MobileInsight icon="🔥" label="Pico" value={horariosMaisVendidos[0] ? `${horariosMaisVendidos[0].nome} · ${horariosMaisVendidos[0].quantidade}x` : 'Sem dados'} />
          </div>
        </div>
        ...
        <div style={mobileCompactPanelSoft}>
          <div style={mobileCompactHeaderLine}>
            ...
            <a href="/promocoes" style={mobileCompactLink}>Campanhas</a>
          </div>

          <div style={mobileRelationshipGrid}>
            <MobileMetric label="Aniversários" value={aniversariantes.length} tone="#f97316" />
            <MobileMetric label="Promoções" value={promocoesAtivas.length} tone="#ec4899" />
          </div>
        </div>
        ...
        <div style={mobileAttentionPanel}>
          <div style={mobileCompactHeaderLine}>
            ...
            <button type="button" onClick={abrirModalReagendamento} style={mobileTextButton}>Ver todas</button>
          </div>
      ...
      <section className="dashboard-analytics-mobile dashboard-desktop-rich" style={analyticsHeroGrid}>
        <div className="dashboard-analytics-card-mobile" style={analyticsMainCard}>
          <div style={analyticsHeaderPremium}>
            <div>
              <span style={sectionKickerPremium}>Financeiro inteligente</span>
              <h2 style={sectionTitlePremium}>Receita, pendências e previsão</h2>
              <p style={sectionDescriptionPremium}>
                Números operacionais do período com visão de caixa recebido, pendências e potencial semanal.
              </p>
            </div>
          ...
          {usuarioPodeVerFinanceiro() ? (
            <>
              <div className="dashboard-finance-grid-mobile" style={financeGridEnterprise}>
                <CardFinanceiro
              ...
              <div className="dashboard-previsao-grid-mobile" style={previsaoGridEnterprise}>
                <div style={previsaoCardEnterprise}>
                  <span>Previsão semanal agendada</span>
                  <strong>{valorFinanceiro(previsaoPeriodo)}</strong>
                  <small>Baseada no valor total dos serviços da semana.</small>
                </div>
                ...
                <div style={previsaoCardEnterprise}>
                  ...
                  <strong>{valorFinanceiro(comissoesPeriodoEstimadas)}</strong>
                  <small>Com base nos atendimentos agendados da semana.</small>
                </div>
              </div>
            </>
          ) : (
            <div style={boxResumoBloqueado}>
              Valores financeiros ocultos. Solicite permissão ao administrador para liberar a visão financeira.
            </div>
          )}
        </div>

        <aside className="dashboard-operacao-card-mobile" style={operacaoLiveCard}>
          ...
          {proximoCliente ? (
            <div style={proximoClientePremium}>
              ...
              <strong>{formatarHorario(proximoCliente.dataHoraInicio)}</strong>
              <h3>{nomeCliente(proximoCliente)}</h3>
              <p>{nomeServico(proximoCliente)}</p>
              <small>👤 {nomeProfissional(proximoCliente)}</small>
            </div>
          ) : (
            <div style={emptyBoxPremium}>Nenhum próximo cliente para hoje.</div>
          )}

          <div style={operacaoMiniGrid}>
            <ResumoItem label="Em atendimento" valor={atendimentosEmAndamento.length} />
            <ResumoItem label="Pendentes" valor={agendamentosPendentesPeriodo.length} />
            <ResumoItem label="Cancelamentos" valor={`${taxaCancelamentoPeriodo}%`} />
            <ResumoItem label="Conclusão" valor={`${taxaConclusaoPeriodo}%`} />
          </div>

          <button onClick={abrirModalReagendamento} style={botaoOperacaoPremium}>
            Abrir central de atendimentos
          </button>
        </aside>
      </section>

      <section className="dashboard-operacional-grid-mobile dashboard-desktop-rich" style={gridOperacionalEnterprise}>
        <div className="dashboard-panel-large-mobile" style={panelEnterpriseLarge}>
          <div style={analyticsHeaderPremium}>
            <div>
              <span style={sectionKickerPremium}>Agenda do dia</span>
              <h2 style={sectionTitlePremium}>Atendimentos de hoje</h2>
              <p style={sectionDescriptionPremium}>
                Lista operacional rápida para confirmação de presença e fechamento.
              </p>
            </div>
          ...
          {agendamentosHoje.length === 0 ? (
            ...
            <div className="dashboard-agenda-lista-mobile" style={agendaListaPremium}>
              {agendamentosHoje.map((agendamento) => {
                const financeiro = resumoFinanceiroAtendimento(agendamento);
                const temPromocaoAplicada = promocaoFoiAplicadaDashboard(agendamento);
                const valorOriginalPromocao = valorNumerico(agendamento?.valorOriginalServico);
                const valorEconomizadoPromocao = valorNumerico(agendamento?.valorEconomizado);

                return (
                  <div key={agendamento.id} style={agendaRowPremium}>
                    <div style={agendaHoraPremium}>{formatarHorario(agendamento.dataHoraInicio)}</div>

                    <div style={{ minWidth: 0 }}>
                      <strong style={agendaClientePremium}>{nomeCliente(agendamento)}</strong>
                    ...
                    <div style={agendaFinanceiroPremium}>
                      {temPromocaoAplicada && valorOriginalPromocao > 0 && (
                        ...
                        >
                          {mostrarFinanceiro ? dinheiro(valorOriginalPromocao) : '••••••'}
                        </span>
                      )}

                      <strong>{mostrarFinanceiro ? dinheiro(financeiro.total) : '••••••'}</strong>
                      <span style={{ color: financeiro.pendente > 0 ? '#f59e0b' : '#22c55e' }}>
                        {financeiro.pendente > 0 ? 'pendente' : 'quitado'}
                      </span>
  ...
  >
    Taxa: {taxaCancelamentoPeriodo}%
              ...
              <div style={smartAlertPremium}>
                ...
                <span>{servicoCampeao ? `${servicoCampeao.nome} · ${servicoCampeao.quantidade}x` : 'Sem volume suficiente ainda.'}</span>
              </div>
            </div>
          </div>
          ...
          <div style={panelEnterprise}>
            ...
            <div className="dashboard-atalhos-grid-mobile" style={atalhosGridPremium}>
              <a href="/clientes" style={atalhoPremium}>+ Agendar</a>
              <a href="/servicos" style={atalhoPremium}>Serviços</a>
              <a href="/profissionais" style={atalhoPremium}>Profissionais</a>
              <a href="/promocoes" style={atalhoPremium}>Promoções</a>
              <a href="/comissoes" style={atalhoPremium}>Comissões</a>
              <a href="/configuracoes" style={atalhoPremium}>Configurações</a>
            </div>
          </div>
        </div>
      </section>

      <section className="dashboard-desktop-rich" style={performanceGridEnterprise}>
        <div style={panelEnterprise}>
          ...
          {servicosRequisitadosSemana.length === 0 ? (
            <div style={emptyBoxPremium}>Nenhum serviço requisitado nesta semana.</div>
          ) : (
            <div style={rankingStackPremium}>
              {servicosRequisitadosSemana.map((servico: any, index: number) => (
        ...
        <div style={panelEnterprise}>
          ...
          {profissionaisRequisitadosSemana.length === 0 ? (
            <div style={emptyBoxPremium}>Nenhum profissional requisitado nesta semana.</div>
          ) : (
            <div style={rankingStackPremium}>
              {profissionaisRequisitadosSemana.map((profissional: any, index: number) => (
        ...
        <div style={panelEnterprise}>
          ...
          {horariosMaisVendidos.length === 0 ? (
            <div style={emptyBoxPremium}>Sem dados suficientes no período.</div>
          ) : (
            <div style={rankingStackPremium}>
          ...
          <div style={miniInsightPremium}>
            ...
            <span>{diaMaisMovimentado ? `${diaMaisMovimentado.nome} · ${diaMaisMovimentado.quantidade} atend.` : 'Sem histórico suficiente'}</span>
          </div>
        </div>
      </section>
        ...
        <div className="dashboard-panel-large-mobile" style={panelEnterpriseLarge}>
          <div style={analyticsHeaderPremium}>
            <div>
              ...
              <h2 style={sectionTitlePremium}>Aniversários e oportunidades</h2>
              <p style={sectionDescriptionPremium}>
                Transforme datas e promoções ativas em retorno de clientes pelo WhatsApp.
              </p>
            </div>
          ...
          <div style={crmColumnsPremium}>
            <div>
              <h3 style={subsectionTitlePremium}>🎂 Aniversariantes da semana</h3>
              {aniversariantes.length === 0 ? (
                <div style={emptyBoxPremium}>Nenhum aniversariante nos próximos 7 dias.</div>
              ) : (
                <div style={crmListPremium}>
                  {aniversariantes.map((cliente: any) => (
                    <div key={cliente.id} style={clienteCardPremium}>
                      ...
                      {cliente.whatsapp && (
                        <a href={`https://wa.me/55${String(cliente.whatsapp).replace(/\D/g, '')}`} target="_blank">
                          <button style={botaoWhatsAppPremium}>WhatsApp</button>
                        </a>
                      )}
                    </div>
                  ))}
            ...
            <div>
              ...
              {promocoesAtivas.length === 0 ? (
                <div style={emptyBoxPremium}>Nenhuma promoção ativa no momento.</div>
              ) : (
                <div style={crmListPremium}>
                  {promocoesAtivas.map((promo: any) => (
                    <div key={promo.id} style={promocaoCardPremium}>
                      ...
                      <small>
                        🎁 {promo.tipoDesconto === 'percentual' ? `${promo.desconto}%` : dinheiro(promo.desconto)} ·{' '}
                        {promo.dataInicio ? new Date(promo.dataInicio).toLocaleDateString('pt-BR') : '--'} até{' '}
      ...
      <section className="dashboard-desktop-rich" style={chartPanelEnterprise}>
        <div style={analyticsHeaderPremium}>
          <div>
...
<p style={sectionDescriptionPremium}>
  Valores recebidos e pré-pagos no período filtrado.
        ...
        <div className="dashboard-chart-mobile"><ResponsiveContainer width="100%" height={230}>
...
/>
      ...
      {modalReagendamentoAberto && (
        <div className="dashboard-modal-overlay-mobile" style={modalOverlay} onClick={fecharModalReagendamento}>
          <div className="dashboard-modal-box-mobile" style={modalBox} onClick={(e) => e.stopPropagation()}>
            <div className="dashboard-modal-header-mobile" style={modalHeader}>
              <div>
                <h2 style={{ margin: 0 }}>Central de atendimento</h2>
                <p style={{ marginTop: 6, color: '#64748b' }}>
                  Pesquise agendamentos abertos por nome, serviço, profissional, WhatsApp ou CPF.
                </p>
              </div>
...
<div className="dashboard-filtros-central-mobile" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 12 }}>
  {[
    ...
    <button
      ...
      style={{
        ...
        cursor: 'pointer',
        color:
          filtroCentralAtendimentos === item.key ? '#fff' : '#cbd5e1',
        background:
    ...
    >
      {item.label}
            ...
            <div className="dashboard-modal-grid-mobile" style={modalGrid}>
              <div className="dashboard-lista-central-mobile" style={listaAgendamentos}>
                {carregandoCentral ? (
                  <div style={emptyBox}>
  ...
  >
    ...
    <div>
      Motivo: {agendamento.motivoCancelamento || 'Não informado'}
                      ...
                      {(() => {
                        const financeiro = resumoFinanceiroAtendimento(agendamento);
                        const ajusteFechamento = resumoAjusteFechamentoSalvo(agendamento);

                        return (
                          <div style={resumoFinanceiroModalLista}>
                            {ajusteFechamento.temAjuste && ajusteFechamento.valorOriginal > 0 && (
                              <span
                                style={{
                                  color: '#94a3b8',
                                  fontSize: 12,
                            ...
                            {ajusteFechamento.temAcrescimo && (
                              <span style={{ color: '#f97316', fontWeight: 950 }}>
              ...
              >
                {!agendamentoSelecionado ? (
                  ...
                  <>
                    ...
                    {agendamentoSelecionado.status !== 'cancelado' ? (
                      <>
<div className="dashboard-fechamento-box-mobile" style={fechamentoBox}>
                        ...
                        return (
                          <>
                            <div style={servicosAdicionaisHeader}>
                              <div>
                                <span style={etapaKickerAzul}>ETAPA 2 • SERVIÇOS E VALORES</span>
                                <strong style={tituloSecaoOperacional}>Financeiro e serviços do atendimento</strong>
                                <p style={descricaoSecaoOperacional}>
                                  Consulte o resumo financeiro, visualize o serviço principal e adicione serviços extras quando necessário.
                                </p>
                              </div>
...
/>
                            ...
                            {promocaoFoiAplicadaDashboard(agendamentoSelecionado) && (
                              <div
                                style={{
                                  marginTop: 16,
                                  padding: 18,
                      ...
                      <div style={servicoPrincipalBox}>
                        <span style={labelModal}>Serviço principal já contratado</span>
                        <strong>{nomeServico(agendamentoSelecionado)}</strong>
                    ...
                    <div className="dashboard-reagendamento-box-mobile" style={reagendamentoOpcionalBox}>
                      <div style={{ marginBottom: 10 }}>
                        <span style={etapaKickerRoxo}>ETAPA 3 • REAGENDAMENTO</span>
                        <strong style={tituloSecaoOperacional}>Reagendar atendimento (opcional)</strong>
                        <p style={descricaoSecaoOperacional}>
                          Use esta área somente se precisar mudar a data ou horário. Para finalizar ou adicionar serviços, não é necessário reagendar.
                        </p>
                      </div>
...
function MobileAttentionItem({ icon, title, description, tone }: any) {
  return (
    <button type="button" style={mobileAttentionItem} onClick={() => {}}>
      <span style={{ ...mobileAttentionIcon, background: `${tone}22`, color: tone }}>
        {icon}
      </span>
...
function CardFinanceiro({ titulo, valor, descricao, cor }: any) {
  return (
    ...
    >
      <div style={{ ...financeiroOrb, background: cor, boxShadow: `0 0 28px ${cor}88` }} />
...
const botaoFiltroEnterprisePrimario: React.CSSProperties = {
  ...
  fontWeight: 950,
  cursor: 'pointer',
  boxShadow: '0 16px 34px color-mix(in srgb, var(--marcae-primary) 30%, transparent)',
};
...
const horaGrande: React.CSSProperties = { fontSize: 42, fontWeight: 950, color: '#fff' };
