...
export default function AgendaPage() {
  ...
  const [periodoAgenda, setPeriodoAgenda] = useState<PeriodoAgenda>("dia");
  ...
  async function carregarAgenda(idEmpresa: string) {
    const res = await fetch(
      `/api/dashboard/agendamentos?empresaId=${idEmpresa}`,
    );
    const data = await res.json();

    if (data.success) {
      const agendamentosUnicos = removerAgendamentosDuplicadosAgenda(
        data.agendamentos || [],
      );
  ...
  function periodoAtivoAgenda(periodo: PeriodoAgenda) {
    return periodoAgenda === periodo;
  }

  function aplicarFiltroPeriodoAgenda(periodo: PeriodoAgenda) {
    const hoje = hojeFormatoInput();

    setPeriodoAgenda(periodo);

    if (!dataFiltroAgenda) {
      setDataFiltroAgenda(hoje);
    }

    if (periodo === "dia") {
      setDataFiltroAgenda(hoje);
      setMostrarSemanaMobile(false);
      return;
    }

    if (periodo === "semana") {
      setDataFiltroAgenda(hoje);
      setMostrarSemanaMobile(true);
      return;
    }
  ...
  const eventosMobileMes = useMemo(() => {
    const dataBase = dataInputParaDate(dataFiltroAgenda);
  ...
  const eventosMobilePeriodo = useMemo(() => {
    if (periodoAgenda === "semana") {
      return eventosMobileSemana;
    }

    if (periodoAgenda === "mes") {
      return eventosMobileMes;
    }

    return eventosMobileDiaSelecionado;
  }, [
  ...
  const metricasMobileDiaSelecionado = useMemo(() => {
    const finalizados = eventosMobilePeriodo.filter(
      (evento) => evento.resource?.status === "concluido",
    ).length;

    const faturamento = eventosMobilePeriodo.reduce((acc, item) => {
      const agendamento = item.resource;
      const totalPago = valorPago(agendamento);
    ...
    return {
      agendamentos: eventosMobilePeriodo.length,
      finalizados,
      faturamento,
    };
  }, [eventosMobilePeriodo]);
  ...
  const tituloPeriodoMobile =
    periodoAgenda === "semana"
      ? `Semana • ${formatarDataCurta(inicioSemanaAgenda)} a ${formatarDataCurta(fimSemanaAgenda)}`
      : periodoAgenda === "mes"
        ? dataSelecionadaMobile.toLocaleDateString("pt-BR", {
            month: "long",
            year: "numeric",
          })
        : formatarDataCurta(dataFiltroAgenda);

  const subtituloPeriodoMobile =
    periodoAgenda === "semana"
      ? `${eventosMobileSemana.length} agendamentos na semana`
      : periodoAgenda === "mes"
        ? `${eventosMobileMes.length} agendamentos no mês`
        : nomeDiaMobile;

  const textoEmptyPeriodo =
    periodoAgenda === "semana"
  ...
  const descricaoEmptyPeriodo =
    periodoAgenda === "semana"
      ? "Use a data para escolher outra semana ou pesquise por cliente, serviço, WhatsApp ou CPF."
      : periodoAgenda === "mes"
        ? "Use a data para escolher outro mês ou pesquise por cliente, serviço, WhatsApp ou CPF."
        : "Use as setas ou toque em outro dia da semana para navegar.";
        ...
        <main className="agenda-page-responsive" style={page}>
          <div className="agenda-container-responsive" style={container}>
            ...
            >
              ...
              <div
                className="agenda-header-conteudo-responsive"
                style={headerConteudoCompacto}
              >
                <div
                  className="agenda-header-identidade-responsive"
                  style={headerIdentidadeAgenda}
                >
                  <div
                    className="agenda-logo-responsive"
                    style={logoHeaderCompacto}
                  >
                    {empresa.logo ? (
                      <img
                        src={empresa.logo}
                  ...
                  <div style={headerTituloBoxCompacto}>
                    <h1
                      className="agenda-titulo-responsive"
                      style={tituloHeaderCompacto}
                    >
                      Agenda
                    </h1>
                    <span style={subtituloHeaderCompacto}>
                      {tituloPeriodoMobile} •{" "}
                      {metricasMobileDiaSelecionado.agendamentos} agendamentos
                    </span>
                  </div>
                </div>

                <div
                  className="agenda-header-acoes-responsive"
                  style={headerAcoesCompactas}
                >
                  <div
                    className="agenda-header-mini-stat-responsive"
                    style={headerMiniStat}
                  >
                    <span>Hoje</span>
              ...
              <div
                className="agenda-badges-responsive agenda-header-info-grid-responsive"
                style={headerInfoGridCompacto}
              >
                <div
                  className="agenda-header-info-card-responsive"
                  style={headerInfoCardCompacto}
                >
                ...
                >
                  ...
                  <div>
                    <strong>
                      {empresa.plano ? `Plano ${empresa.plano}` : "Plano ativo"}
                    </strong>
                    <small>
            ...
            <section
              className="agenda-metricas-responsive"
              style={metricasGrid}
            >
              <MetricCard
                titulo="Hoje"
                valor={String(metricas.hoje)}
                descricao="Agenda"
                icone="📅"
                cor={corPrimaria}
              />
              <MetricCard
                titulo="Concluídos"
                valor={String(metricas.concluidos)}
                descricao="Semana"
                icone="✅"
                cor="#22c55e"
              />
            ...
            <section
              className="agenda-toolbar-responsive"
              style={toolbarAgendaCompacto}
            >
              <div style={toolbarFiltrosRapidos}>
                <button
                  type="button"
                  onClick={() => aplicarFiltroPeriodoAgenda("dia")}
                  style={{
                    ...botaoFiltroRapidoAgenda,
                    ...(periodoAtivoAgenda("dia")
                      ? {
                ...
                <button
                  type="button"
                  onClick={() => aplicarFiltroPeriodoAgenda("semana")}
                  style={{
                    ...botaoFiltroRapidoAgenda,
                    ...(periodoAtivoAgenda("semana")
                      ? {
                          background: `linear-gradient(135deg, ${corPrimaria}, ${corSecundaria})`,
                          color: "#fff",
                          borderColor: "rgba(255,255,255,0.18)",
                ...
                <button
                  type="button"
                  onClick={() => aplicarFiltroPeriodoAgenda("mes")}
                  style={{
                    ...botaoFiltroRapidoAgenda,
                    ...(periodoAtivoAgenda("mes")
                      ? {
                          background: `linear-gradient(135deg, ${corPrimaria}, ${corSecundaria})`,
                          color: "#fff",
                          borderColor: "rgba(255,255,255,0.18)",
              ...
              <div
                className="agenda-toolbar-busca-data-responsive"
                style={toolbarBuscaDataCompacta}
              >
                <div
                  className="agenda-busca-compacta-responsive"
                  style={buscaCompactaBox}
                >
                  ...
                  {pesquisa && (
                    <button
                      onClick={() => setPesquisa("")}
                      style={botaoLimparBuscaCompacto}
                    >
                      ×
                    </button>
                  )}
                ...
                <div
                  className="agenda-data-compacta-responsive"
                  style={dataCompactaBox}
                >
                  <button
                    type="button"
                  ...
                  <div
                    className="agenda-input-data-label-responsive"
                    style={inputDataCompactaLabel}
                  >
                    <input
              ...
              <div
                className="agenda-periodo-responsive"
                style={periodoAgendaTextoCompacto}
              >
                Semana • {formatarDataCurta(inicioSemanaAgenda)} a{" "}
                {formatarDataCurta(fimSemanaAgenda)}
              </div>
            </section>
                  ...
                  return diasSemana.map((dia) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={cardsDia}>
                          {eventosDia.length === 0 ? (
                            <div style={diaVazio}>Nenhum agendamento</div>
                          ) : (
                                  ...
                                  <div style={cardConteudo}>
                                    ...
                                    <div style={badgesCard}>
                                      ...
                                      <span style={badgeProfissional}>
                                        👤{" "}
                                        {a.profissional?.nome || "Profissional"}
                                      </span>

                                      <span
                                        style={{
                                          ...badgePagamento,
                                          background: pagamentoConfirmadoAgenda(
                                            a,
                                          )
                                            ? "rgba(34,197,94,0.12)"
                                    ...
                                    <div style={rodapeCard}>
                                      ...
                                      <strong>
                                        {dinheiro(a.valorTotal || a.valor || 0)}
                                      </strong>
                                    </div>
                                  </div>
                                </button>
            ...
            <section className="mobile-lista" style={mobileLista}>
              {mostrarSemanaMobile && (
                <div
                  className="agenda-mobile-dia-card-responsive"
                  style={mobileAgendaDiaCard}
                >
                  <div style={mobileAgendaDiaTopo}>
                    ...
                    <div style={mobileTituloDiaAtualBox}>
                      ...
                      <span style={mobileSubtituloDiaAtual}>
                        {subtituloPeriodoMobile}
                      </span>
                    </div>
                  ...
                  <div
                    className="agenda-mobile-dias-scroller-responsive"
                    style={mobileDiasSemanaScroller}
                  >
                  ...
                  <div
                    className="agenda-mobile-resumo-grid-responsive"
                    style={mobileResumoDiaGrid}
                  >
                    <div style={mobileResumoDiaItem}>
                      <strong>
                        {metricasMobileDiaSelecionado.agendamentos}
                      </strong>
                    ...
                    <div style={mobileResumoDiaItem}>
                      <strong>
                        {dinheiro(metricasMobileDiaSelecionado.faturamento)}
                      </strong>
                      <span>Recebido</span>
                    </div>
                    ...
                    <div style={mobileResumoDiaItem}>
                      <strong>
                        {metricasMobileDiaSelecionado.finalizados}
                      </strong>
                  ...
                  <div
                    className="agenda-mobile-dica-responsive"
                    style={mobileDicaArraste}
                  >
                    <button
                      type="button"
                      onClick={() => navegarDiaMobile("anterior")}
                    ...
                    <span>
                      Deslize ou use as setas para navegar entre os dias
                    </span>
              ...
              {eventosMobilePeriodo.length === 0 ? (
                <div style={emptyState}>
                  <strong>{textoEmptyPeriodo}</strong>
                  <span>{descricaoEmptyPeriodo}</span>
                </div>
              ) : (
                eventosMobilePeriodo.map((evento) => {
                  const a = evento.resource;
                    ...
                    >
                      <div style={mobileCardLinhaPrincipal}>
                        <div style={mobileHoraBox}>
                          <span
                            className="agenda-mobile-hora-responsive"
                            style={mobileHora}
                          >
                            {formatarHora(a.dataHoraInicio)}
                          </span>
                          <small style={mobileData}>
                            {formatarDataCurta(a.dataHoraInicio)}
                          </small>
                        </div>

                        <div style={mobileCardInfo}>
                          <strong style={mobileClienteNome}>
                            {a.cliente?.nome || a.nomeCliente || "Cliente"}
                          </strong>
                          <small style={mobileServicoNome}>
                            {a.servico?.nome || a.servicoNome || "Serviço"}
                          </small>

                          <div
                            className="agenda-mobile-card-rodape-responsive"
                            style={mobileCardRodape}
                          >
                            <span>
                              👤 {a.profissional?.nome || "Profissional"}
                            </span>
                      ...
                      <div style={mobileStatusLinha}>
                        ...
                        {a.reagendadoVisual && (
                          <span style={badgeReagendado}>🔁 Reagendado</span>
                        )}
                        ...
                        {promocaoFoiAplicada(a) && (
                          <span style={badgePromocaoAplicada}>🎁 Promoção</span>
                        )}
                      </div>
                    </button>
                  );
          ...
          {eventoSelecionado && (
            <div
              className="agenda-drawer-overlay-responsive"
              style={drawerOverlay}
              onClick={() => setEventoSelecionado(null)}
            >
              <aside
                className="agenda-drawer-responsive"
                style={drawer}
              ...
              >
                <div style={drawerHeaderCompacto}>
                  <div style={drawerHeaderTextoCompacto}>
                    ...
                    <h2
                      className="agenda-drawer-title-responsive"
                      style={drawerTitleCompacto}
                    >
                      {eventoSelecionado.cliente?.nome || "Cliente"}
                    </h2>
                    <p style={drawerSubtitleCompacto}>
                      {eventoSelecionado.servico?.nome || "Serviço"} •{" "}
                      {formatarHora(eventoSelecionado.dataHoraInicio)}
                    </p>
                  </div>
                ...
                <div style={drawerResumoCompacto}>
                  <div style={drawerResumoLinha}>
                    ...
                    <div style={drawerResumoTexto}>
                      <small>Cliente</small>
                      <strong>
                        {eventoSelecionado.cliente?.nome || "Não informado"}
                      </strong>
                    </div>
                  </div>
                  ...
                  <div style={drawerResumoLinha}>
                    ...
                    <div style={drawerResumoTexto}>
                      <small>Serviço</small>
                      <strong>
                        {eventoSelecionado.servico?.nome || "Não informado"}
                      </strong>
                    </div>
                  </div>
                  ...
                  <div style={drawerResumoLinha}>
                    ...
                    <div style={drawerResumoTexto}>
                      ...
                      <strong>
                        {eventoSelecionado.profissional?.nome ||
                          "Não informado"}
                      </strong>
                    </div>
                  </div>
                ...
                <div style={drawerMetaGridCompacto}>
                  <div style={drawerMetaCardCompacto}>
                    ...
                    <strong>
                      {formatarData(eventoSelecionado.dataHoraInicio)}
                    </strong>
                  </div>
                ...
                {eventoSelecionado.status === "cancelado" && (
                  <div style={drawerCancelamentoCompacto}>
                    <div>
                      ...
                      <strong>
                        {eventoSelecionado.motivoCancelamento ||
                          "Não informado"}
                      </strong>
                    </div>
                    ...
                    <div>
                      ...
                      <strong>
                        {formatarCanceladoEm(eventoSelecionado.canceladoEm)}
                      </strong>
                    </div>
                  </div>
                )}
                ...
                <div style={drawerFinanceiroCompacto}>
                  ...
                  <div style={drawerFinanceiroListaCompacta}>
                    <div style={drawerFinanceiroLinhaCompacta}>
                      <small>Status</small>
                      <strong>
                        {pagamentoConfirmadoAgenda(eventoSelecionado)
                          ? "Pago"
                          : eventoSelecionado.statusPagamento || "Pendente"}
                      </strong>
                    </div>
                    ...
                    <div style={drawerFinanceiroLinhaCompacta}>
                      ...
                      <strong>
                        {formatarMetodoPagamento(eventoSelecionado)}
                      </strong>
                    </div>
                  </div>
                </div>
                ...
                {promocaoFoiAplicada(eventoSelecionado) && (
                  <div style={promoDrawerCard}>
                    ...
                    <div
                      className="agenda-promo-grid-responsive"
                      style={promoDrawerGrid}
                    >
                      <div>
                        <small>Tipo</small>
                        <strong>
                      ...
                      <div>
                        ...
                        <strong>
                          {dinheiro(
                            Number(eventoSelecionado.valorEconomizado || 0),
                          )}
                        </strong>
                      </div>
                    </div>
                ...
                <div style={drawerAcoesGridCompacto}>
                  ...
                  <button
                    onClick={() => setEventoSelecionado(null)}
                    style={botaoFecharDrawerCompacto}
                  >
                    Fechar
                  </button>
                </div>
                  ...
                  eventoSelecionado.status !== "cancelado" && (
                    <div style={reagendamentoBox}>
                      ...
                      <p style={reagendamentoTexto}>
                        No painel administrativo, a empresa pode reagendar mesmo
                      ...
                      <button
                        ...
                        style={botaoCancelarReagendamento}
                      >
...
const logoHeaderCompacto: CSSProperties = {
  ...
  overflow: "hidden",
  background:
    "linear-gradient(135deg, rgba(124,58,237,0.92), rgba(168,85,247,0.92))",
