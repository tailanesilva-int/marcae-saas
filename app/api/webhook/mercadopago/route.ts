...
import { enviarWhatsapp, montarMensagemConfirmacao } from '@/lib/whatsapp';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const tipoEvento = body.type || searchParams.get('type');
    const paymentId =
      body.data?.id ||
      searchParams.get('data.id') ||
      searchParams.get('id');

    if (tipoEvento !== 'payment' || !paymentId) {
    ...
    const tipoPagamento = metadata.tipo || metadata.type;
    ...
    // 💳 ASSINATURA
    // =============================
    if (tipoPagamento === 'assinatura' && empresaId) {
      ...
      const pagamentoAssinaturaExistente = preferenceId
      ...
      console.log('✅ Assinatura confirmada:', empresaId);
      return NextResponse.json({ received: true });
    }
    ...
    if (agendamentoId) {
      ...
      await prisma.pagamento.updateMany({
        ...
        data: {
          ...
          metodoPagamento: payment.payment_method_id || null,
          valorPago: payment.transaction_amount || 0,
          paidAt: new Date(),
        },
      });
      ...
      // 📲 WHATSAPP AUTOMÁTICO
      // =============================
      const planoEmpresa = agendamento.empresa?.plano;
      const isTrial = agendamento.empresa?.trialAtivo;

      const podeEnviarWhatsapp = planoEmpresa !== 'basico' || isTrial;

      if (!podeEnviarWhatsapp) {
        console.log('🚫 Plano básico - não envia WhatsApp automático');
        return NextResponse.json({ received: true });
      }

      if (!agendamento.dataHoraInicio) {
        console.log('⚠️ Agendamento sem data/hora - WhatsApp não enviado');
        return NextResponse.json({ received: true });
      }

      try {
        const mensagem = montarMensagemConfirmacao({
          nomeCliente: agendamento.cliente?.nome || 'Cliente',
          nomeServico: agendamento.servico?.nome || 'Serviço',
          nomeEmpresa: agendamento.empresa?.nome || 'Empresa',
          ...
          slugEmpresa: agendamento.empresa?.slug,
          whatsappEmpresa: agendamento.empresa?.whatsapp,
          enderecoEmpresa: agendamento.empresa?.endereco,
        });
      ...
      console.log('⚠️ Não encontrou agendamentoId');
    }
