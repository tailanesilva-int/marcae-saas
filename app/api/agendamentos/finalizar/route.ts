  ...
  try {
    ...
    if (agendamento.status === 'cancelado') {
      return NextResponse.json(
        {
          success: false,
          error: 'Atendimento cancelado não pode ser finalizado.',
        },
        { status: 400 }
      );
    }
    ...
    */
    ...
    if (dataAtendimento.getTime() > agora.getTime()) {
      return NextResponse.json(
        {
          success: false,
          error: 'Atendimentos futuros não podem ser finalizados.',
        },
        { status: 400 }
      );
    }
    ...
    const principalPago = pagamentoFoiRealizado(
      agendamento.statusPagamento
    );

    let totalPago = principalPago ? valorPrincipal : 0;

    let totalPendente = principalPago ? 0 : valorPrincipal;

    const servicosAdicionais = (
      (agendamento as any).servicosAdicionais || []
    ) as any[];
    ...
    if (quitarPendencias) {
      ...
      await (prisma as any).agendamentoServico.updateMany({
        ...
        data: {
          statusPagamento: 'pago',
          formaPagamento:
            pagamentos?.map((p: any) => p.forma).join(', ') || null,
          pagoEm: new Date(),
        },
      });

      totalPago = numero(agendamento.valorTotal);
    ...
    if (!pagamentoExistente) {
      await prisma.pagamento.create({
        data: {
          empresaId,
          agendamentoId: agendamento.id,
          valorTotal: numero(agendamento.valorTotal),
          valorPago: totalPago,
          metodoPagamento:
            pagamentos
              ?.map(
                (p: any) =>
                  `${p.forma}: R$ ${Number(p.valor || 0).toFixed(2)}`
              )
              .join(' | ') || 'não informado',
